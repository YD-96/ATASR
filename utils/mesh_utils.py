...
def post_process_mesh(mesh, cluster_to_keep=1000):
    ...
    n_cluster = max(n_cluster, 1000) # filter meshes smaller than 50
...
class GaussianExtractor(object):
    ...
    def export_image(self, path):
        ...
        for idx, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="export images"):
            ...
            save_img_u8(gt.permute(1,2,0).cpu().numpy(), os.path.join(gts_path, '{0:04d}'.format(idx+1) + ".jpg"))
            save_img_u8(self.rgbmaps[idx].permute(1,2,0).cpu().numpy(), os.path.join(render_path, '{0:04d}'.format(idx+1) + ".jpg"))
            save_img_f32(self.depthmaps[idx][0].cpu().numpy(), os.path.join(vis_path, 'depth_{0:04d}'.format(idx+1) + ".tiff"))
            save_img_f32(self.alphamaps[idx][0].cpu().numpy(), os.path.join(alpha_path, 'alpha_{0:04d}'.format(idx+1) + ".tiff"))
            save_img_u8(self.normals[idx].permute(1,2,0).cpu().numpy() * 0.5 + 0.5, os.path.join(vis_path, 'normal_{0:04d}'.format(idx+1) + ".jpg"))
            save_img_u8(self.depth_normals[idx].permute(1,2,0).cpu().numpy() * 0.5 + 0.5, os.path.join(vis_path, 'depth_normal_{0:04d}'.format(idx+1) + ".jpg"))

            import matplotlib.pyplot as plt
            depth_array = self.depthmaps[idx][0]
            depth_min = depth_array.min()
            depth_max = depth_array.max()
            depth_normalized = (depth_array - depth_min) / (depth_max - depth_min)
            depth_image = (depth_normalized * 255)
            plt.imshow(depth_image, cmap='plasma_r')
            plt.axis('off')
            plt.savefig(os.path.join(vis_path, '{0:04d}'.format(idx+1) + ".jpg"), bbox_inches='tight', pad_inches=0)
            np.save(os.path.join(vis_path, '{0:04d}'.format(idx+1) + ".npy"), depth_array)

    @torch.no_grad()
    def export_image_f(self, path):
        render_path = os.path.join(path, "renders")
        gts_path = os.path.join(path, "gt")
        vis_path = os.path.join(path, "vis")
        alpha_path = os.path.join(path, "alpha")
